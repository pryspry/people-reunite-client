    ...
    <q-page class="q-pa-md" v-if="!$apollo.queries.naskahs.loading">
        <div class="row container ppl_home">
            <div class="col-12 ppl_borderbottom q-mb-sm">
                <p class="text-h6 text-uppercase"><q-icon name="fas fa-bolt" /> Latest News</p>
            </div>
            <div class="col-12 q-py-md" v-for="naskah in naskahs" :key="naskah.id">
                <router-link :to="`/post/` + naskah.slug" class="ppl_home_router">
                    <div class="row">
                        <div class="col-4">
                            <q-img v-if="`https://people.xabi.us${naskah.Cover.url}` !== null" placeholder-src="https://via.placeholder.com/480x300.png/333/ccc/?text=%23peoplereunite" :src="`https://people.xabi.us${naskah.Cover.url}`" :ratio="1/1" />
                        </div>
